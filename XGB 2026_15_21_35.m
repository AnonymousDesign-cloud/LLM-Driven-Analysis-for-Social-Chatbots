clc;clear;close all;	
load('R_19_May_2026_15_21_35.mat')	
...
Mdl=train_xgb(train_x_feature_label_norm,train_y_feature_label,paramters);	
	
	
y_train_predict=round(predict_xgb(Mdl,train_x_feature_label_norm)); %训练集预测结果	
y_train_predict(y_train_predict<min(train_y_feature_label))=min(train_y_feature_label);	
y_train_predict(y_train_predict>max(train_y_feature_label))=max(train_y_feature_label);	
	
y_vaild_predict=round(predict_xgb(Mdl,vaild_x_feature_label_norm)); %验证集预测结果	
y_vaild_predict(y_vaild_predict<min(train_y_feature_label))=min(train_y_feature_label);	
y_vaild_predict(y_vaild_predict>max(train_y_feature_label))=max(train_y_feature_label);	
	
y_test_predict=round(predict_xgb(Mdl,test_x_feature_label_norm)); %测试集预测结果	
y_test_predict(y_test_predict<min(train_y_feature_label))=min(train_y_feature_label);	
y_test_predict(y_test_predict>max(train_y_feature_label))=max(train_y_feature_label);	
...
%% 绘制ROC曲线	
[score_train1]=predict_xgb(Mdl,train_x_feature_label_norm);  %训练集预测结果	
[score_vaild1]=predict_xgb(Mdl,vaild_x_feature_label_norm);  %验证集预测结果	
[score_test1]=predict_xgb(Mdl,test_x_feature_label_norm);  %测试集预测结果	
	
score_train=exp(1-score_train1);score_train(score_train>1)=1;	
score_vaild=exp(1-score_vaild1);score_vaild(score_vaild>1)=1;	
score_test=exp(1-score_test1);score_test(score_test>1)=1;	
