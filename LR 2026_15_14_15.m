clc;clear;close all;	
load('R_19_May_2026_15_14_15.mat')	
...
disp('Logistic二分类')	
t1=clock; 	
Xtr = train_x_feature_label_norm;  	
Ytr = train_y_feature_label;   	
if max(Ytr)>1  	
     Ytr=Ytr-1;  	
end  	
Xva = vaild_x_feature_label_norm; Xte = test_x_feature_label_norm;  	
[b, dev, stats] = glmfit(Xtr, Ytr, 'binomial', 'link', 'logit');  	
	
p_train = glmval(b, Xtr, 'logit');  p_vaild = glmval(b, Xva, 'logit'); p_test  = glmval(b, Xte, 'logit'); 	
disp('Logistic系数(标准化特征)：'); disp(b')  	
disp('各系数p值(含截距)：'); disp(stats.p)  	
thr = 0.6;%阈值 默认0.6  	
y_train_predict = double(p_train >= thr)+1;  	
y_vaild_predict = double(p_vaild >= thr)+1;  	
y_test_predict  = double(p_test  >= thr)+1;  	
Mdl{1,1} = b;  Mdl{1,2}=thr;  	
t2=clock; 	
...
%% 绘制ROC曲线	
score_train=1-p_train;	
score_vaild=1-p_vaild;	
score_test=1-p_test;	
