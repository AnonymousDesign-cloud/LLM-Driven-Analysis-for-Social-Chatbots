clc;clear;close all;	
load('R_19_May_2026_15_07_08.mat')	
...
y_vaild_predict=predict(Mdl,vaild_x_feature_label_norm);  %验证集预测结果	
...
for k = 1:Kfold_num	
   ...
   y_feature_label_norm_all_testk=y_feature_label_norm_all(validationIdx,:);	
     ...
     Mdlkf=fitctree(x_feature_label_norm_all_traink,y_feature_label_norm_all_traink);	
	
   Mdl_kfold{1,k}=Mdlkf;	
	
    y_test_predict_all_testk=predict(Mdlkf,x_feature_label_norm_all_testk);  %测试集预测结果	
